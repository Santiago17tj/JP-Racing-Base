...
/// Pantalla para editar todos los detalles de una orden generada.
class EditarOrdenScreen extends StatefulWidget {
  final OrdenMantenimiento ordenActual;
  const EditarOrdenScreen({super.key, required this.ordenActual});
  @override
...
class _EditarOrdenScreenState extends State<EditarOrdenScreen> {
  ...
  Future<void> _cargarDatosIniciales() async {
    ...
    final cliente = await provider.obtenerClienteDeOrden(widget.ordenActual.clienteId);
    final vehiculos = await provider.obtenerVehiculosDeCliente(widget.ordenActual.clienteId);
    final vehiculo = vehiculos.firstWhere((v) => v.id == widget.ordenActual.vehiculoId);
    ...
    setState(() {
      ...
      _kilometrajeCtrl.text = widget.ordenActual.kilometrajeIngreso.toString();
      _problemaCtrl.text = widget.ordenActual.descripcionProblema ?? '';
      _mecanicoCtrl.text = widget.ordenActual.mecanicoAsignado ?? '';
      
      if (TiposServicio.valores.contains(widget.ordenActual.tipoServicio)) {
        _tipoServicio = widget.ordenActual.tipoServicio;
      } else {
        _tipoServicio = TiposServicio.otro;
        _tipoServicioCustomCtrl.text = widget.ordenActual.tipoServicio;
      }
    });
  }
  ...
  Future<void> _guardarOrden() async {
    ...
    try {
      final String tipoServicioFinal = _tipoServicio == TiposServicio.otro ? _tipoServicioCustomCtrl.text.trim() : _tipoServicio;

      if (_esNuevoCliente) {
        messenger.showSnackBar(const SnackBar(content: Text('Por favor, edite clientes en la sección de Clientes')));
        ...
        await provider.actualizarOrdenCompleta(
          widget.ordenActual.id,
          clienteId:           _clienteSeleccionado!.id,
          vehiculoId:          _vehiculoSeleccionado!.id,
          kilometraje:         int.tryParse(_kilometrajeCtrl.text) ?? 0,
  ...
  Widget _buildNuevoClienteForm() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding: const EdgeInsets.all(AppTheme.spacingMd), decoration: AppTheme.cardDecoration, child: Column(children: [
        TextFormField(controller: _nombreCtrl,    decoration: const InputDecoration(labelText: 'Nombre'), validator: _req),
        const SizedBox(height: 12),
        TextFormField(controller: _apellidoCtrl,  decoration: const InputDecoration(labelText: 'Apellido'), validator: _req),
        const SizedBox(height: 12),
        TextFormField(controller: _documentoCtrl, decoration: const InputDecoration(labelText: 'Documento / Cédula'), keyboardType: TextInputType.number, validator: _req),
        const SizedBox(height: 12),
    ...
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: _subiendoFotos
            ...
            : const Text('GUARDAR CAMBIOS', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1.2)),
      ),
    );
  }
